...
var MBImport = (function () {
    ...
    function fnBuildSearchLink(release) {
        ...
        return `<a class="musicbrainz_import" href="//bandcamp.com/search?${url_params.join('&')}">Search on Bandcamp</a>`;
    }

    // compute HTML of search button
    function fnBuildSearchButton(release) {
        let parameters = searchParams(release);
        let html = `<form class="musicbrainz_import musicbrainz_import_search" action="//bandcamp.com/search" method="get" target="_blank" accept-charset="UTF-8" charset="${document.characterSet}">`;
        parameters.forEach(function (parameter) {
            let value = `${parameter.value}`;
            html += `<input type='hidden' value='${value.replace(/'/g, '&apos;')}' name='${parameter.name}'/>`;
        });
        html += '<button type="submit" title="Search for this release in MusicBrainz (open a new tab)"><span>Search on Bandcamp</span></button>';
        html += '</form>';
        return html;
    }
    ...
    function fnSearchUrlFor(type, what) {
        ...
        let params = [`q=${luceneEscape(what)}`, `type=${type}`, 'indexed=1'];
        return `//bandcamp.com/search?${params.join('&')}`;
    }
    ...
    function searchParams(release) {
        ...
        appendParameter(params, 'q', query);
